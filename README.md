...
- **Problem framing**: Shows ability to decompose business questions into intent taxonomy and SQL templates.
- **System design**: Multi-tenant architecture with clean isolation (dataset-per-tenant, RLS/CLS, service account impersonation).
- **RACI definition**: Clear ownership (Solutions Architect: taxonomy, glossary, prompts, SLOs; Data Engineer: API, validators, pipelines; Data Analyst: golden set curation, business acceptance).

### For MLE / Data Engineer
- **Validators & guardrails**: SQL linting (`sql/sql_policies.yaml`), allowlist enforcement, cost controls.
- **Tests & CI**: Unit tests (`tests/`), golden set evaluation (`eval/`), promotion gates (`ops/gates.yaml`).
- **Observability**: Trace bundles for replayability, monitoring dashboards (`ops/monitoring_dashboards.json`), runbook (`ops/runbook.md`).

### For Data Scientist (NLP)
- **NL→SQL pipeline**: Router classifies intent, planner grounds measures/dimensions in glossary, SQL emitter fills templates.
- **Eval metrics**: Golden set pass rate, faithfulness score, citation coverage, SQL execution accuracy.
- **Golden sets**: Curated test cases with canonical phrasing, paraphrases, and adversarial examples (`eval/golden_set/`).

This repo becomes your primary link in applications.
